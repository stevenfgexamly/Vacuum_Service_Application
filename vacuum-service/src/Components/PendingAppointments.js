...
function ViewAppointments() {
    ...
    const getPendingAppointments = () => {
        axios.get("http://localhost:8080/api/getPendingAppointments")
    ...
    const completeAppointment = (aid) => () => {
        axios.get("http://localhost:8080/api/completeAppointment?aid=" + aid)
            .then(response => {
    ...
    return (
        <>
            <Card className='bg-dark text-white' >
                <Card.Header>
                    <center>
                        <label style={{ fontSize: "1.5rem" }} >Confirm Service Completion</label>
                    </center>
                </Card.Header>
            </Card>
            <Container>
                <Row>
                    {
                        appointments == 0 ?
                            <Card className='bg-dark text-white m-2'>
                                <Card.Header>
                                    <Card.Title style={{ fontSize: "30px" }} >No Pending Appointments</Card.Title>
                                </Card.Header>
                                ...
                                <Card key={appointment.aid} className='bg-dark text-white m-2'>
                                    ...
                                    <Card.Body className='bg-dark text-white' >
                                        <Card.Text style={{ fontSize: "20px" }}>
                                            ...
                                            <Row>
                                                <Col>
                                                    <Button onClick={completeAppointment(appointment.aid)} size="lg" variant="success" style={{ marginRight: '1rem' }} className='float-left' type="button">Confirm Completion</Button>
                                                </Col>
                                                <Col>
            ...
            <div style={{ "display": show ? "block" : "none" }}>
                <CustomToast show={show} message={"Appointment Completion Confirmed"} type={"success"}></CustomToast>
            </div>
        </>
    );
}
